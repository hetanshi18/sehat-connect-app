...
CREATE TABLE IF NOT EXISTS public.patients_info (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS public.doctors_info (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS public.symptoms_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  patient_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
...
DROP TRIGGER IF EXISTS handle_patients_info_updated_at ON public.patients_info;
CREATE TRIGGER handle_patients_info_updated_at BEFORE UPDATE ON public.patients_info
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS handle_doctors_info_updated_at ON public.doctors_info;
...
DROP POLICY IF EXISTS "Patients can view own info" ON public.patients_info;
CREATE POLICY "Patients can view own info" ON public.patients_info
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Patients can insert own info" ON public.patients_info;
CREATE POLICY "Patients can insert own info" ON public.patients_info
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Patients can update own info" ON public.patients_info;
CREATE POLICY "Patients can update own info" ON public.patients_info
  FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Doctors can view patient info for their appointments" ON public.patients_info;
...
DROP POLICY IF EXISTS "Everyone can view doctors info" ON public.doctors_info;
CREATE POLICY "Everyone can view doctors info" ON public.doctors_info
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Doctors can insert own info" ON public.doctors_info;
CREATE POLICY "Doctors can insert own info" ON public.doctors_info
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Doctors can update own info" ON public.doctors_info;
CREATE POLICY "Doctors can update own info" ON public.doctors_info
  FOR UPDATE USING (auth.uid() = user_id);

-- RLS Policies for time_slots
DROP POLICY IF EXISTS "Everyone can view available slots" ON public.time_slots;
CREATE POLICY "Everyone can view available slots" ON public.time_slots
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Doctors can insert own slots" ON public.time_slots;
CREATE POLICY "Doctors can insert own slots" ON public.time_slots
  FOR INSERT WITH CHECK (auth.uid() = doctor_id);

DROP POLICY IF EXISTS "Doctors can update own slots" ON public.time_slots;
CREATE POLICY "Doctors can update own slots" ON public.time_slots
  FOR UPDATE USING (auth.uid() = doctor_id);

DROP POLICY IF EXISTS "Doctors can delete own slots" ON public.time_slots;
...
DROP POLICY IF EXISTS "Patients can view own symptoms" ON public.symptoms_records;
CREATE POLICY "Patients can view own symptoms" ON public.symptoms_records
  FOR SELECT USING (auth.uid() = patient_id);

DROP POLICY IF EXISTS "Patients can insert own symptoms" ON public.symptoms_records;
CREATE POLICY "Patients can insert own symptoms" ON public.symptoms_records
  FOR INSERT WITH CHECK (auth.uid() = patient_id);

DROP POLICY IF EXISTS "Doctors can view patient symptoms for their appointments" ON public.symptoms_records;
...
DROP POLICY IF EXISTS "Doctors can view patient health records for their appointments" ON public.health_records;
...
DROP POLICY IF EXISTS "Users can view messages for their appointments" ON public.messages;
...
DROP POLICY IF EXISTS "Users can insert messages for their appointments" ON public.messages;
